          ...
          <tbody>
            {adding && (
              <tr style={{ background: '#e6f7ff' }}>
                <td><label>Date{<RequiredAsterisk />}<br /><input type="date" name="date" value={addForm.date} onChange={handleAddChange} /></label></td>
                <td><label>Source{<RequiredAsterisk />}<br /><input type="text" name="source" value={addForm.source} onChange={handleAddChange} /></label></td>
                <td><label>Points Used{<RequiredAsterisk />}<br /><input type="number" name="points" value={addForm.points} onChange={handleAddChange} min="1" /></label></td>
                <td><label>Additional Taxes/Fees Paid ($)<br /><input type="number" name="taxes" value={addForm.taxes} onChange={handleAddChange} min="0" step="0.01" /></label></td>
                <td><label>Total Cash Value ($){<RequiredAsterisk />}<br /><input type="number" name="value" value={addForm.value} onChange={handleAddChange} min="0.01" step="0.01" /></label></td>
                <td>{addForm.points > 0 && addForm.value !== '' && addForm.taxes !== '' ? (((addForm.value - addForm.taxes) / addForm.points) * 100).toFixed(1) + '¢/pt' : ''}</td>
                <td><label>Notes<br /><input type="text" name="notes" value={addForm.notes} onChange={handleAddChange} /></label></td>
                <td>
                  <button onClick={handleAddSave} disabled={!isAddValid} style={{ color: isAddValid ? 'green' : 'gray', marginRight: 4 }}>Add</button>
                  <button onClick={handleAddCancel} style={{ color: 'gray' }}>Cancel</button>
                </td>
              </tr>
            )}
            <tr>
              <td colSpan={8} style={{ padding: 0, border: 'none', background: 'none' }}>
                {!adding && (
            ...
            {pagedData.map(r => editingId === r.id ? (
              <tr key={r.id} style={{ background: '#f9f9f9' }}>
                ...
                <td><input type="text" name="source" value={editForm.source} onChange={handleEditChange} /></td>
              ...
              <tr key={r.id}>
                ...
                <td>{r.notes}</td>
                <td>
                  <button onClick={() => handleEdit(r)} style={{ color: 'blue', marginRight: 4 }}>Edit</button>
                  <button onClick={() => handleDelete(r.id)} disabled={deleting === r.id} style={{ color: 'red' }}>
